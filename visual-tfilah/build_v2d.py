...
PLAN = [
    ...
    ("Amidah", "Mi Shebeirach", ("NEW", "misheb1")),
    ("Amidah", "Mi Shebeirach", ("NEW", "misheb2")),
...
NEW = {
    ...
    "misheb1": dict(
        ...
        translation="[ English refrain (Debbie Friedman) — paste in the school's slide ]",
        translation_dim=True,
        notes="Mi Shebeirach 1 of 2 (Debbie Friedman). Hebrew invocation, first couplet. The English healing refrain is Friedman's copyrighted lyric — placeholder only; paste in the school's slide.",
    ),
    "misheb2": dict(
        en="Mi Shebeirach", he_title="מִי שֶׁבֵּרַךְ",
        translit=["Mi shebeirach imoteinu,", "m'kor hab'rachah la'avoteinu."],
        hebrew=["מִי שֶׁבֵּרַךְ אִמּוֹתֵינוּ,", "מְקוֹר הַבְּרָכָה לַאֲבוֹתֵינוּ."],
        translation="[ English refrain (Debbie Friedman) — paste in the school's slide ]",
        translation_dim=True,
        notes="Mi Shebeirach 2 of 2 (Debbie Friedman). Hebrew invocation, second couplet. English healing refrain placeholder — paste in the school's slide.",
    ),
    "shofar": dict(
...
def place_kiddush_cup(work, order_files, ct_path):
    """Persist the wine cup in the column gutter on every Kiddush slide.

    v1d only had the cup on Kiddush 1, sitting low where Kiddush 2 and 3 have text.
    The gutter between the transliteration (left) and Hebrew (right) columns is
    empty on all three, so the cup lives there — same place and size throughout.
    """
    CUP = "v1d_7_image5.png"                       # already in the package from the import
    X, Y, W, H = 6419088, 3017520, 1627632, 2834463   # 7.02", 3.30", 1.78 x 3.10"
    for f in order_files:
        sp = f"{work}/ppt/slides/{f}"
        x = open(sp, encoding="utf-8").read()
        # read the English header box specifically — the nav also lists "Kiddush"
        hdr = next((m.group(0) for m in re.finditer(r"<p:sp>.*?</p:sp>", x, re.S)
                    if 'name="TextBox 15"' in m.group(0)), "")
        if "".join(re.findall(r"<a:t[^>]*>(.*?)</a:t>", hdr, re.S)).strip() != "Kiddush":
            continue
        rp = f"{work}/ppt/slides/_rels/{f}.rels"
        rels = open(rp, encoding="utf-8").read()
        # Drop any existing cup. A <p:pic> names its image only by r:embed id, and
        # the filename lives in the rels — so resolve the cup's rId(s) first, then
        # remove the pics that embed them (a filename match on the pic XML never hits).
        cup_rids = set(re.findall(r'Id="(rId\d+)"[^>]*Target="\.\./media/' + CUP.replace(".", r"\.") + '"', rels))
        for rid in cup_rids:
            x = re.sub(r'<p:pic>(?:(?!</p:pic>).)*?r:embed="' + rid +
                       r'"(?:(?!</p:pic>).)*?</p:pic>', "", x, flags=re.S)
        rid = "rId%d" % (max(int(r[3:]) for r in re.findall(r'Id="(rId\d+)"', rels)) + 1)
        if CUP not in rels:
            rels = rels.replace("</Relationships>",
                                f'<Relationship Id="{rid}" Type="http://schemas.openxmlformats.org/'
                                f'officeDocument/2006/relationships/image" Target="../media/{CUP}"/>'
                                "</Relationships>")
            open(rp, "w", encoding="utf-8").write(rels)
        else:
            rid = re.search(r'Id="(rId\d+)"[^>]*' + CUP.replace(".", r"\."), rels).group(1)
        pic = (f'<p:pic><p:nvPicPr><p:cNvPr id="951" name="Kiddush Cup"/><p:cNvPicPr/>'
               f'<p:nvPr/></p:nvPicPr><p:blipFill><a:blip r:embed="{rid}"/><a:stretch>'
               f'<a:fillRect/></a:stretch></p:blipFill><p:spPr><a:xfrm><a:off x="{X}" y="{Y}"/>'
               f'<a:ext cx="{W}" cy="{H}"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/>'
               f'</a:prstGeom></p:spPr></p:pic>')
        x = x.replace("</p:spTree>", pic + "</p:spTree>", 1)
...
def global_fixes(x):
    ...
    # Leading space belongs to a paragraph, not a run. Strip it from the FIRST run
    # of each Hebrew paragraph only — stripping every run once welded Ya'eir to
    # Adonai in the Priestly Blessing, whose inter-word space sits at a run start.
    def para_lead(m):
        pblk = m.group(0)
        first = re.search(r"<a:r>(?:(?!</a:r>).)*</a:r>", pblk, re.S)
        if not first:
            return pblk
        run0 = first.group(0)
        if not any("֐" <= c <= "׿" for c in "".join(re.findall(r"<a:t[^>]*>(.*?)</a:t>", run0, re.S))):
            return pblk
        fixed = re.sub(r"(<a:t[^>]*>)[ \xa0]+", r"\1", run0, count=1)
        return pblk[:first.start()] + fixed + pblk[first.end():]
    x = re.sub(r"<a:p>.*?</a:p>", para_lead, x, flags=re.S)

    # A run that still starts or ends with a space needs xml:space="preserve", or
    # the renderer drops it — that space separates Ya'eir from Adonai. para_lead
    # already trimmed unwanted leading indents, so what remains here is real.
    def preserve(m):
        attrs, text = m.group(1) or "", m.group(2)
        if (text[:1].isspace() or text[-1:].isspace()) and "xml:space" not in attrs:
            return '<a:t%s xml:space="preserve">%s</a:t>' % (attrs, text)
        return m.group(0)
    x = re.sub(r"<a:t( [^>]*)?>(.*?)</a:t>", preserve, x, flags=re.S)
    ...
    # Body text boxes: generous fixed height, no autofit, so PowerPoint's slightly
    # taller real fonts can never clip the last line (the Kiddush 3 symptom).
    def grow(m):
        b = m.group(0)
        nm = re.search(r'name="([^"]*)"', b)
        if not nm or nm.group(1) not in ("TextBox 10", "TextBox 11", "Hebrew Body"):
            return b
        b = re.sub(r'(<a:ext cx="\d+" cy=")\d+', lambda mm: mm.group(1) + "8686800", b, count=1)  # 9.5"
        b = b.replace("<a:spAutoFit/>", "").replace("<a:normAutofit/>", "")
        return b
    x = re.sub(r"<p:sp>.*?</p:sp>", grow, x, flags=re.S)
...
CHROME = "".join(sh for sh in top_level_shapes(_auth)
                 if re.search(r'name="(Group 2|Group 6|Freeform 5)"', sh))
...
def fill_new(path, d):
    ...
    x = set_shape_text(x, "TextBox 18",
                       para(d["translation"], RPR_EN.format(sz=2799, b="",
                            col=(DIM if d.get("translation_dim") else "FFFFFF"))))
...
def fill_hold(path, key):
    ...
    x = set_shape_text(x, "TextBox 12", para(he, RPR_HE.format(sz=4200, b="", col="FFFFFF"),
                                             algn="r", rtl=True) if he else "<a:p/>")
...
LOV_HEADER = (
    '<p:sp><p:nvSpPr><p:cNvPr id="915" name="TextBox 15"/><p:cNvSpPr txBox="1"/>'
    '<p:nvPr/></p:nvSpPr><p:spPr><a:xfrm><a:off x="182880" y="64008"/>'
    '<a:ext cx="9144000" cy="557784"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/>'
    '</a:prstGeom><a:noFill/></p:spPr><p:txBody>'
    '<a:bodyPr lIns="0" tIns="0" rIns="0" bIns="0" rtlCol="0" anchor="ctr"/><a:lstStyle/>'
    '<a:p><a:pPr algn="l"/><a:r><a:rPr lang="en-US" sz="3199" b="1">'
    '<a:solidFill><a:srgbClr val="FFFFFF"/></a:solidFill>'
    '<a:latin typeface="Calibri (MS)"/><a:cs typeface="Calibri (MS)"/></a:rPr>'
    '<a:t>Living Our Values Everyday</a:t></a:r></a:p></p:txBody></p:sp>'
)


def add_crowd_logo(x, rels_path):
    rels = open(rels_path, encoding="utf-8").read()
    existing = re.search(r'Id="(rId\d+)"[^>]*Target="\.\./media/image8\.png"', rels)
    if existing:
        ...
        rels = rels.replace("</Relationships>",
                            f'<Relationship Id="{rid}" Type="http://schemas.openxmlformats.org/'
                            f'officeDocument/2006/relationships/image" Target="../media/image8.png"/>'
                            "</Relationships>")
        open(rels_path, "w", encoding="utf-8").write(rels)
    lx, ly, lw, lh = 15511536, 7791450, 2222500, 2222500     # 16.96", 8.52", 2.43 sq — matches every other slide
    pic = (f'<p:pic><p:nvPicPr><p:cNvPr id="916" name="Davis Logo"/><p:cNvPicPr/>'
           f'<p:nvPr/></p:nvPicPr><p:blipFill><a:blip r:embed="{rid}"/><a:stretch>'
           f'<a:fillRect/></a:stretch></p:blipFill><p:spPr><a:xfrm><a:off x="{lx}" y="{ly}"/>'
           f'<a:ext cx="{lw}" cy="{lh}"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/>'
           f'</a:prstGeom></p:spPr></p:pic>')
    return x.replace("</p:spTree>", pic + "</p:spTree>", 1)
...
for sec, item, src in PLAN:
    ...
    if src == ("T", 3):
        # Living Our Values: keep the five value words, but drop the centre logo
        # (a chrome logo goes in the corner) and lift the title into the header
        # bar so the slide reads like the rest of the deck.
        x = re.sub(r'<p:sp>(?:(?!</p:sp>).)*?name="Freeform 6".*?</p:sp>', "", x, flags=re.S)  # centre crowd mark
        x = re.sub(r'<p:sp>(?:(?!</p:sp>).)*?name="TextBox 7".*?</p:sp>', "", x, flags=re.S)   # L.O.V.E. body title
        x = re.sub(r'<p:sp>(?:(?!</p:sp>).)*?name="Freeform 9".*?</p:sp>', "", x, flags=re.S)  # academy logo (blipFill)
        x = rescale(x, 16.15 / 18.63, dy=274320)
        x = x.replace("</p:spTree>", CHROME + LOV_HEADER + "</p:spTree>", 1)
        x = add_crowd_logo(x, f"{WORK}/ppt/slides/_rels/{fname}.rels")
...
fname_of = {v: k for k, v in rid_of.items()}
place_kiddush_cup(WORK, [fname_of[r] for r in order_rids], ct_path)
print("placed the wine cup in the gutter on every Kiddush slide")
